...
_LINEAR_massive, _BINARY_massive, _DIRECT_string = [], [], ""  # створюються масиви та рядок, де будуть проводитись
# пошуки
searchLinear, searchBinary, searchDirect = int(), int(), str()  # створюються типи даних, які будуть шукатись
...
if rand_or_inp_LINEAR == 1:  # користувач вказує масив чисeл
    _LINEAR_massive = list(map(int, input('\nВведіть цілі числа через пробіл:\n>>> ').split()))
elif rand_or_inp_LINEAR == 2:  # генерується масив із рандомних чисел
...
if search_LINEAR_inp == 1:  # вводиться число, яке потрібно знайти
    searchLinear = int(input('\nВведіть число:\n>>> '))
elif search_LINEAR_inp == 2:  # генерується випадкове число
...
rand_or_inp_BINARY = int(input('\nВведіть:\n'
                               '1 - якщо хочете ввести дані самостійно\n'
                               '2 - якщо хочете заповнити дані радномно\n>>> '))
if rand_or_inp_BINARY == 1:  # користувач вказує масив чисeл
    _BINARY_massive = sorted(list(map(int, input('\nВведіть числа через пробіл:\n>>> ').split())))
elif rand_or_inp_BINARY == 2:  # генерується масив із рандомних чисел
    _BINARY_massive = sorted([random.randint(-10, 10) for i in range(int(input('\nВведіть кількість чисел:\n>>> ')))])

search_BINARY_inp = int(input('\nВведіть:\n'
                              '1 - якщо хочете самостійно ввести число, яке потрібно знайти\n'
                              '2 - якщо хочете згенерувати будь-яке число в діапазоні [-10; 10]\n>>> '))
if search_BINARY_inp == 1:  # вводиться число, яке потрібно знайти
    searchBinary = int(input('\nВведіть число:\n>>> '))
elif search_BINARY_inp == 2:  # генерується випадкове число
...
if rand_or_inp_DIRECT == 1:  # користувач вводить текст та рядок, який потрібно знайти в тексті
    _DIRECT_string = input('\nВведіть текст:\n>>> ')
    searchDirect = input('Введіть підрядок: ')
elif rand_or_inp_DIRECT == 2:  # пирсвоюються вже зазначені дані
    _DIRECT_string = 'eggsspag spar egspasspameggs esagsspa'
    searchDirect = 'spameggs'

# |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

linear = linearSearch(_LINEAR_massive, searchLinear)  # виконуються операції функцій
binary = binarySearch(_BINARY_massive, searchBinary)  # алгоритмів, які повертають список із
direct = directSearch(_DIRECT_string, searchDirect)  # індексу елементу та к-сті порівнянь

print(f'\nЛІНІЙНИЙ ПОШУК\n'
      f'Ваш масив даних: {_LINEAR_massive}')  # виведення даних на екран
print(f'Ваше шукане число {searchLinear} знаходиться на позиції {linear[0]}\n'
      f'Кількість порівнянь: {linear[1]}')
timeLinear = timeit.timeit("linearSearch(_LINEAR_massive, searchLinear)",  # підраховується час на пошук
                           setup="from __main__ import linearSearch, _LINEAR_massive, searchLinear", number=1)
print(f'Час, що витратився: {timeLinear:.4e} с.')

print(f'\nБІНАРНИЙ ПОШУК\n'
      f'Ваш масив даних: {_BINARY_massive}')  # виведення даних на екран
print(f'Ваше шукане число {searchBinary} знаходиться на позиції {binary[0]}\n'
      f'Кількість порівнянь: {binary[1]}')
timeBinary = timeit.timeit("binarySearch(_BINARY_massive, searchBinary)",  # підраховується час на пошук
                           setup="from __main__ import binarySearch, _BINARY_massive, searchBinary", number=1)
print(f'Час, що витратився: {timeBinary:.4e} с.')

print(f'\nПРЯМИЙ ПОШУК ПІДРЯДКА\n'
      f'Ваш текст: {_DIRECT_string}')  # виведення даних на екран
...
timeBinary = timeit.timeit("directSearch(_DIRECT_string, searchDirect)",  # підраховується час на пошук
